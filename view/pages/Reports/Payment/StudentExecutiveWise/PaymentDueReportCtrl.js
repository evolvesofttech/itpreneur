...
controller("studentplacementCtrl", 
    ...
    $window.document.title = "ITPreneur - Studentwise/Executivewise Payment Due Report";
   ...
   $scope.branchChange = function(id, branchdata) {
    ...
    //} else {
        ...
        var urlExeList = baseUrlSrvc.baseUrl + 'listAccountUsersReportByBranchId/' + id;
   ...
   $scope.executiveChange = function(id, data) {
    ...
    $scope.temp_exe = $filter('filter')(data, { "adUserId": id });
     ...
     var urlExeStuList = baseUrlSrvc.baseUrl + 'listEnquiryFormByadUserId/' + id;
      var promiseExeStuGet = CRUD_SERVICE.getAllData(urlExeStuList);
      promiseExeStuGet.then(function(pl) {
   ...
   //    var urlExeStuList = baseUrlSrvc.baseUrl + 'listEnquiryFormByadUserId/' + $sessionStorage.adUserId;
   //    var promiseExeStuGet = CRUD_SERVICE.getAllData(urlExeStuList);
   //    promiseExeStuGet.then(function(pl) {
   //        $scope.AllStudents = pl.data;
   //    },
   ...
   =            student Change            =
   ========================================*/
   $scope.studentChange = function(id, data) {
    if (id != undefined) {
    $scope.temp_exe1 = $filter('filter')(data, { "eEnquiryFormId": id });
     ////console.log("$scope.temp_exe1",$scope.temp_exe1);
     $scope.stuName = $scope.temp_exe1[0].firstname + ' ' + $scope.temp_exe1[0].middlename + ' ' + $scope.temp_exe1[0].lastname; 
     $scope.stuId = $scope.temp_exe1[0].eEnquiryFormId;
     ////console.log("$scope.stuId",$scope.stuId);
   }
     else {
       $scope.stuId = "null";
     }
   ...
   $scope.generateReport = function(data) {
        ...
        data.eEnquiryFormId = [{"eEnquiryFormId":$scope.stuId}];

        if (data.eEnquiryFormId[0].eEnquiryFormId == "null") {
          $scope.stuName = "All";
        }
        ...
        var FormData = {
            ...
            url: baseUrlSrvc.baseUrl + 'listPaymentDueStudentwiseOrExecutivewise'
        };
        ...
        promisePut.then(function (pl) {
            ...
            ////console.log("StudentPlacementReportData",pl.data);
            ...
            if ($scope.paymentDueData.fromdate == "null") {
              $scope.paymentDueData.fromdate = "";
            }
            if ($scope.paymentDueData.todate == "null") {
              $scope.paymentDueData.todate = "";
            }
